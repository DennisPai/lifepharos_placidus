...
#include "swephexp.h"   /* Swiss Ephemeris header */
...
#define EPHE_PATH "./swisseph-master/ephe" /* 星曆表路徑 */
...
int main(int argc, char *argv[])
{
    ...
    FILE *jpl_file = fopen(EPHE_PATH "/de441.eph", "rb");
    if (jpl_file != NULL) {
        fclose(jpl_file);
        printf("找到JPL星曆表(DE441)，使用最高精確度計算\n");
        ephemeris_type = SEFLG_JPLEPH;
        swe_set_jpl_file(EPHE_PATH "/de441.eph");
    } else {
        jpl_file = fopen(EPHE_PATH "/de431.eph", "rb");
        if (jpl_file != NULL) {
            fclose(jpl_file);
            printf("找到JPL星曆表(DE431)，使用最高精確度計算\n");
            ephemeris_type = SEFLG_JPLEPH;
            swe_set_jpl_file(EPHE_PATH "/de431.eph");
        } else {
            /* 檢查瑞士星曆表 */
            FILE *swiss_file = fopen(EPHE_PATH "/semo_00.se1", "r");
            if (swiss_file != NULL) {
                fclose(swiss_file);
                printf("找到瑞士星曆表，使用精確計算\n");
                ephemeris_type = SEFLG_SWIEPH;
            } else {
                printf("未找到星曆表文件，使用內建近似計算\n");
    ...
    /* 創建資料夾並保存資料 */
    sprintf(folder_name, "%d_%d_%d_%d_%d_%.1f_%.1f", year, month, day, hour, minute, longitude, latitude);
    ...
    sprintf(html_copy, "%s/%d_%d_%d_%d_%d_%.1f_%.1f.html", 
...
void save_txt_data(char *folder_name, PlanetInfo *planets, HouseInfo *houses, AspectInfo *aspects, int aspect_count, 
    ...
    /* 建立檔案名稱 */
    sprintf(filename, "%s/%d_%d_%d_%d_%d_%.1f_%.1f.txt", 
            folder_name, year, month, day, hour, minute, longitude, latitude);
    ...
    fprintf(fp, "時區: UTC%+d\n", estimate_timezone(longitude));
    
    /* 計算世界時的儒略日（考慮時區） */
    double jd_ut = swe_julday(year, month, day, hour + minute / 60.0, SE_GREG_CAL);
    double local_time = hour + minute / 60.0;
    double delta_t = swe_deltat(jd_ut); // Delta T in days
    double sid_time;
